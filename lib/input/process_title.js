...
const process_title = Object.defineProperty( input, process_titleSymbol, {
    ...
    value: async function* selector ( process_parsed_argv ){
                ...
                for ( const flag in process_parsed_argv.keys ) {
                    ...
                    switch ( process_parsed_argv.keys[ flag ] ) {
                        ...
                        case 'test': {
                            ...
                            delete optionsObject.command.unit
                            ...
                            for ( const flag in initFlags ) {
                                ...
                                switch ( initFlags[ flag ] ) {
                                    ...
                                    case 'file': {
                                        ...
                                        let { filename, error } = await options( process_parsed_argv.object[ initFlags[ flag ] ], '--file='.green().underline() )

                                        if( error instanceof Error )
                                            await process_exit( error.message, Error( `${ process.title } flags-error` ), shell_exit_codes.flags )

                                        optionsObject.command.test.file.filename = filename
                        ...
                        case 'unit': {
                            ...
                            delete optionsObject.command.test
                            delete process_parsed_argv.object.unit
                            process_parsed_argv.keys.splice( 0, 1 )

                            const twd = `${ process.cwd() }/${ await twd__.get() }`
                            optionsObject.command.unit.twd = await readdir( twd ).then( directory_listing => directory_listing.filter( dot => !/(^|\/)\.[^/.]/g.test( dot ) ) )
                            ...
                            for ( const flag in unitFlags ) {
                                ...
                                switch ( unitFlags[ flag ] ) {
                                    ...
                                    case 'exclude': {
                                        ...
                                        optionsObject.command.unit.exclude = process_parsed_argv.object[ unitFlags[ flag ] ].split( ',' )
        ...
        if( process_title_not_recognize ) {
            ...
            await process_exit( error, new ReferenceError( 'internal-error' ), shell_exit_codes.internal )
        }
