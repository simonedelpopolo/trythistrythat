...
const separatorSymbol = Symbol.for( 'Object [ tttt.output.separator ]' )
const separator = Object.defineProperty( output, separatorSymbol, {
    enumerable: true,
    writable: false,
    configurable: false,
     ...
     */
    value: async function separator( color = undefined, length = 75, char = '-' ) {
        ...
        if( ! color ) console.log( separator_text.b_cyan() )

        else {
            if( await number_( color, undefined, undefined, false )
                && color >= 0 && color <= 255 )
                console.log( separator_text.color( color ) )
            else
...
export default separator[ separatorSymbol ]
